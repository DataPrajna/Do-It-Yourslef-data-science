...
class LinearRegressor:
    """
     ...
     Examples:
          ...
          let set the initial W and b using a random number
           ...
           >>> lr.set_wb_parameter(W = numpy.random.randn(1, 1), b = numpy.random.randn(1, 1))
    ...
    def set_wb_parameter(self, W=None, b=None):
        ...
        if W is None:
            self.W = tf.Variable(numpy.random.randn(self.num_features,1), dtype=tf.float32, name="w")
        else:
            self.W = tf.Variable(W, name="W")
        if b is None:
            self.b = tf.Variable(numpy.random.randn(),dtype=tf.float32,  name="b")
        else:
    ...
    def train(self, train_X=None, train_Y=None):
        ...
        solver = self.solver();
        batch_data = BatchDataServer(train_X, train_Y, batch_size = 100)
        with tf.Session() as sess:
            sess.run(tf.initialize_all_variables())
...
def test_linear_regressor():
    lr = LinearRegressor(lr=0.000001, num_epocs=10000, print_frequency=100, num_features=3)
    train_X = numpy.asarray(
        [3.3, 4.4, 5.5, 6.71, 6.93, 4.168, 9.779, 6.182, 7.59, 2.167, 7.042, 10.791, 5.313, 7.997, 5.654, 9.27, 3.1],
        dtype=numpy.float32)
    train_X = train_X.reshape((17, 1))
    train_X = numpy.column_stack([train_X, train_X*train_X, train_X*train_X*train_X])
    print(train_X.shape)
    lr.set_wb_parameter()
    ...
    with tf.Session() as sess:
        sess.run(tf.initialize_all_variables())
        y = sess.run(lr.predictor(), feed_dict={lr.X:train_X})

    W, b = lr.train(train_X=train_X, train_Y=train_Y)
    #lr.set_wb_parameter(W=W, b=b)
    #y_hat = lr.predict(X=train_X)
    #error = lr.error(X=train_X, Y = train_Y)
    #print(y_hat)
    #print(error)
